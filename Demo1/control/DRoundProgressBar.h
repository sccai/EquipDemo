...
#ifndef DROUNDPROGRESSBAR_H
#define DROUNDPROGRESSBAR_H

#include <QWidget>

/**
 * @brief The DRoundProgressBar class represents a circular progress bar and maintains its API
 * similar to the *QProgressBar*.
 *
 * ### Styles
 * DRoundProgressBar currently supports Donut, Pie and Line styles. See setBarStyle() for more details.
 *
 * ### Colors
 * Generally DRoundProgressBar uses its palette and font attributes to define how it will look.
 *
 ...
 *
 * ### Value text
 * Value text is generally drawn inside the DRoundProgressBar using its `font()` and \a QPalette::Text role from its `palette()`.
 *
 ...
 */
class DRoundProgressBar : public QWidget
{
    Q_OBJECT
public:    
    explicit DRoundProgressBar(QWidget *parent = 0);
...
protected:
    ...
    virtual void drawBackgroundInner(QPainter& p, const QRectF& baseRect);
    virtual void drawBase(QPainter& p, const QRectF& baseRect, const QRectF& baseRectInner);
    virtual void drawBaseInner(QPainter& p, const QRectF& baseRect);
   ...
   void rebuildDataBrushIfNeededInner();
    double m_min, m_max;
    double m_value;
