...
class QDESIGNER_WIDGET_EXPORT BatteryNew : public QWidget
#else
class BatteryNew : public QWidget
#endif
...
public:
    explicit BatteryNew(QWidget *parent = 0);
    ~BatteryNew();
...
private:
    ...
    double maxValue;                //最大值
    double value;                   //目标电量
    double midalarmValue;           //电池电量警戒值,轻微告警
    double alarmValue;              //电池电量警戒值,严重告警
    ...
    QColor midalarmColorStart;      //电池低电量时的渐变开始颜色
    QColor midalarmColorEnd;        //电池低电量时的渐变结束颜色
...
public Q_SLOTS:
    ...
    //设置默认样式,defvalue:0 颜色全部填充；defvalue:1 容器空白；
    void setDefault(int defvalue);
