...
public:
    static FontAweSomeIcons& Instance();//获取类指针

    QFont getFont();//获取字体

    QChar getIconChar(FontAweSomeIcons::IconIdentity code);//获取图标

protected:
    FontAweSomeIcons();

    FontAweSomeIcons(const FontAweSomeIcons& other) = delete;//删除默认拷贝构造函数

    FontAweSomeIcons& operator =(const FontAweSomeIcons& other) = delete;//删除默认拷贝构造函数
